...
def over_under_when_team_is_home(competition, team, spread):
    ...
    for game in football_collection.find({"competition": competition, "home": team}):
        games += 1
        if game["home_ft"] + game["away_ft"] > spread:
            win += 1
        elif game["home_ft"] + game["away_ft"] == spread:
    ...
    if games == 0:
        print(f"There are no data for {team.upper()} in {competition.upper()} as home team.")
    else:
        print(f"Games of {team.upper()} in {competition.upper()} as home team: {games}.")
        print(f"Over {spread}: {win/games*100:.4}%")
        print(f"Equal {spread}: {void/games*100:.4}%")
        print(f"Under {spread}: {lost/games*100:.4}%")
...
def over_under_when_team_is_away(competition, team, spread):
    ...
    for game in football_collection.find({"competition": competition, "away": team}):
        games += 1
        if game["home_ft"] + game["away_ft"] > spread:
            win += 1
        elif game["home_ft"] + game["away_ft"] == spread:
    ...
    if games == 0:
        print(f"There are no data for {team.upper()} in {competition.upper()} as away team.")
    else:
        print(f"Games of {team.upper()} in {competition.upper()} as away team: {games}.")
        print(f"Over {spread}: {win/games*100:.4}%")
        print(f"Equal {spread}: {void/games*100:.4}%")
        print(f"Under {spread}: {lost/games*100:.4}%")
