...
def home_winning(competition):
    ...
    print(f"Home: {home/games*100:.4}%")
    print(f"Away: {away/games*100:.4}%")


def home_spread(competition, spread):
    games = 0
    home = 0
    void = 0
    away = 0
...
def home_win_percentage(competition, handicap):
    games = 0
    win = 0
    void = 0
    lost = 0
...
def team_home_win_percentage(competition, handicap, team):
    ...
    for game in baseball_collection.find({"competition": competition, "home": team}):
        games += 1
        if game["home_ft"] + handicap > game["away_ft"]:
            win += 1
        elif game["home_ft"] + handicap == game["away_ft"]:
...
# competition_spread(competition)
# totals_competition(competition)
team_home_win_percentage(competition, handicap, team)
# competitions_home_winning()