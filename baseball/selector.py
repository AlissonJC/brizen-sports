...
def handicap(competition, home, away, handicap_home):
    ...
    away_win, away_void, away_lost, games_away = team_handicap_as_away(competition, away, -handicap_home)
    ...
    if games_home >= 3 and games_away >= 3:
        if (home_win + home_void) >= likelihood and (away_lost + away_void) >= likelihood:
            if (home_win + away_lost + home_void + away_void) / 2 >= likelihood and (
                    home_win + away_lost + home_void + away_void) / 2 > \
...
def flags(competition):
    ...
    return f"{country[0].capitalize()} {country_name}\n"
...
def calculator(handicap_found, handicap_home, team):
    ...
    if handicap_home >= 0:
        if handicap_home == 0:
            print(f"**{team.title()} para vencer**\n")
